...
var treeDataLayer, addTreeLayer, deleteTreeLayer; // Define separate layers
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Create separate layers for tree data, addition, and deletion
  ...
  fetch("http://localhost:5000/delete_tree", {
    ...
    .then((data) => {
      if (data.status === "success") {
        ...
        deleteTreeLayer.eachLayer(function (layer) {
          if (layer.options.treeId === treeId) {
            deleteTreeLayer.removeLayer(layer);
          }
        });
        fetchInitialTreeData(); // Refresh tree data after deletion
      } else {
...
function createAndAddMarker(treeData) {
  ...
  marker.addTo(treeDataLayer); 
  marker.addTo(deleteTreeLayer);
}